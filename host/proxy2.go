...
// Proxy2 - key value pairs of a URI -> HttpHandler
type Proxy2 struct {
	m *sync.Map
}

// NewProxy2 - create a new Proxy2
func NewProxy2() *Proxy2 {
	p := new(Proxy2)
	p.m = new(sync.Map)
	return p
}
...
func (p *Proxy2) Register(uri string, handler core.HttpExchange) error {
	if len(uri) == 0 {
...
func (p *Proxy2) Lookup(uri string) core.HttpExchange {
	nid, _, ok := uri2.UprootUrn(uri)
	if !ok {
...
func (p *Proxy2) LookupByNID(nid string) core.HttpExchange {
	v, ok := p.m.Load(nid)
	if !ok {
