...
func ExampleNewBasicResource() {
	r := NewBasicResource[core.Origin]("github/advanced-go/stdlib", originMatch2, finalize)
	...
	if !status.OK() {
		...
		resp, _ := r.Do(req)
		fmt.Printf("test: PutT() -> [status-code:%v] [header:%v] [count:%v] [%v]\n", resp.StatusCode, resp.Header, r.Count(), list)
	}
	...
	//Output:
	//test: PutT() -> [status-code:200] [header:map[X-Authority:[github/advanced-go/stdlib] X-Method:[PUT]]] [count:3] [[]]
